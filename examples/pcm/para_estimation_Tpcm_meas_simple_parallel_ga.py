...
if __name__ == "__main__":
    ...
    res_dir = os.path.join('results_test_modestga','modestga','six process')
    ...
    session = Estimation(res_dir, fmu_file, inp, known, est, ideal,
            lp_n=3, lp_frame=(trn_t0, trn_t1),
            vp=(vld_t0, vld_t1),
            methods=('MODESTGA',),
            modestga_opts={
                'generations': 40,   # Max. number of generations
                'pop_size': 30,      # Population size
                'trm_size': 7,       # Tournament size
                'tol': 1e-7,         # Absolute tolerance
                'workers': 6         # Number of CPUs to use
            },
